...
export async function enviarReporte() {
  ...
  try {
    const especieTexto = state.especieEsOtra
      ? 'Otras: ' + (state.especieDescripcion || 'No especificado')
      : state.especie;

    const data = new FormData();
    data.append('form-name', CONFIG.formName);
    data.append('_subject', CONFIG.emailAsunto + ': ' + especieTexto + ' · Ramales');
    data.append('_replyto', state.email || 'sin-email@ejemplo.com');
    data.append('Especie', especieTexto);
    data.append('Fecha', new Date().toISOString().slice(0, 10));
    data.append('Latitud', state.lat || '');
    data.append('Longitud', state.lon || '');
    data.append('Google Maps', state.lat ? `https://maps.google.com/?q=${state.lat},${state.lon}` : '—');
    data.append('Lugar descripcion', state.lugarDesc || '—');
    data.append('Tamanyo colonia', state.tamanyo || '—');
    data.append('Observaciones', state.observaciones || '—');
    data.append('Nombre ciudadano', state.nombre || 'Anónimo');
    data.append('Email ciudadano', state.email || '—');
    ...
    const resp = await fetch(CONFIG.submitUrl, { method: 'POST', body: data });
    if (!resp.ok) throw new Error('Respuesta no válida del servidor');

    return true;
  } catch (err) {
    errorEl.textContent = 'No se ha podido enviar el reporte. Comprueba tu conexión e inténtalo de nuevo.';
